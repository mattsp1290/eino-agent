...
func streamAgentic(ctx context.Context, client einomodel.AgenticModel, req Request) (*einoschema.StreamReader[StreamDelta], error) {
	...
	return einoschema.StreamReaderWithConvert(upstream, func(message *einoschema.AgenticMessage) (StreamDelta, error) {
		...
		return StreamDelta{Message: message, Usage: UsageFromAgenticMessage(message)}, nil
	}), nil
}
