...
func TestSessionWatchRuntimeTerminalPaths(t *testing.T) {
	for _, mode := range []string{"empty", "model-error", "model-panic", "persistence-error", "tool-error", "tool-denied", "tool-cancel"} {
		t.Run(mode, func(t *testing.T) {
			...
			admission, err := orchestrator.Start(ctx, Request{SessionID: "watched", Message: TextUserMessage("user"), Config: orchestratorConfig()})
			if err != nil {
				t.Fatal(err)
			}
			if mode == "tool-cancel" {
				...
				if err = admission.Interrupt(ctx, "fixture"); err != nil {
					t.Fatal(err)
				}
			}
			var result Result
			...
			for {
				...
				for _, r := range u.Snapshot.Runs {
					if r.ID == admission.RunID() && r.Terminal() {
						terminal = true
					}
				}
				if terminal {
