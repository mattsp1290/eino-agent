...
// block objects ADK itself is tracking must be untouched, since ADK's own
// tracked conversation state (compose.GetToolCallID) and this package's own
// store.GetToolCall lookups stay keyed on the durable id.
...
func TestPublicizeToolCallIDsLaterDurableIDEqualToEarlierProviderIDRenamesEarlierCall(t *testing.T) {
	...
	second := append(append([]*einoschema.AgenticMessage(nil), first...),
		toolCallBlockMessage("tool-call-2", "echo", `{}`), toolResultBlockMessage("tool-call-2", "echo"))
	out1, err := publicizeToolCallIDs(context.Background(), store, "session-1", nil, nil, first)
	if err != nil {
		t.Fatal(err)
	}
	out2, err := publicizeToolCallIDs(context.Background(), store, "session-1", nil, nil, second)
	if err != nil {
		t.Fatal(err)
	}
...
func TestPublicizeToolCallIDsFailsClosedOnMissingRow(t *testing.T) {
	...
	_, err := publicizeToolCallIDs(context.Background(), store, "session-1", nil, nil, messages)
...
func TestPublicizeToolCallIDsSentinelScopeAndCause(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err := publicizeToolCallIDs(context.Background(), store, "session-1", nil, nil,
...
func TestPublicizeToolCallIDsCacheAvoidsRepeatedStoreReadsPerDispatch(t *testing.T) {
	...
	for _, batch := range [][]*einoschema.AgenticMessage{first, second, third} {
		if _, err := publicizeToolCallIDs(context.Background(), counting, "session-1", &cache, nil, batch); err != nil {
			t.Fatal(err)
		}
	}
	if reads != 2 {
