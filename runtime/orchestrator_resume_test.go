...
func TestRunHeartbeatPreventsResumeAcrossInjectedClockSkew(t *testing.T) {
	...
	admission, err := owner.Start(testCtx, Request{SessionID: "heartbeat-session", Message: TextUserMessage("wait"), Config: orchestratorConfig()})
	if err != nil {
		t.Fatal(err)
	}
	select {
	...
	initial, err := store.GetRun(testCtx, admission.RunID())
	if err != nil {
		t.Fatal(err)
	}
	deadline := time.Now().Add(2*leaseDuration + time.Second)
	for {
		current, getErr := store.GetRun(testCtx, admission.RunID())
		if getErr != nil {
			t.Fatal(getErr)
		}
		now := time.Now()
	...
	if _, err := resumer.Resume(testCtx, admission.RunID()); !errors.Is(err, session.ErrSessionBusy) {
